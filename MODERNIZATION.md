...
* **First-Class Linux & Cross-Platform Support**: Linux (x86_64 and aarch64) is the primary HPC/production target; Apple Silicon macOS and Windows are supported via portable CMake configurations.
...
|:---|:---|:---|
| **SPOOLES 2.2** | Active (Comparison) | Legacy direct sparse solver. Linux uses system packages; macOS can build the pinned vendor archive for compatibility testing. |
...
| **MUMPS 5.x** | Active (Primary Candidate) | Modern direct sparse solver with OpenMP, Out-of-Core, and Block Low-Rank (BLR) memory reduction. Linux/Windows use system packages; macOS builds the pinned vendor archive. |
| **PaStiX** | Exists in source | High-performance direct solver with GPU acceleration support (`-DPASTIX_GPU`). |
| **arpack-ng** | Active | Upstream-maintained Arnoldi eigenvalue solver for modal dynamic, frequency, and buckling analysis. Replaces legacy ARPACK 96. |
...
- [x] Modern `CMakeLists.txt` for the serial `spooles` static library.
- [x] Reproducible macOS SPOOLES 2.2 vendor archive with checksum.
...
- [x] Official multi-solver coverage runner with isolated solver binaries and result diagnostics.
