...
void steadystate(double **cop,ITG *nk,ITG **konp,ITG **ipkonp,char **lakonp,ITG *ne,
  ...
  ITG nev,i,j,k,*inum=NULL,nsectors,im,intscheme=0,*ibodysav=NULL,
    ...
    *ikactmechr=NULL,*ikactmechi=NULL,nactmechr,nactmechi,intpointvar,
    iforc,iload,ne0,*iponoelt=NULL,*inoelt=NULL,*imdelem=NULL,
  ...
  *nzl=0;
  ...
  if(!cyclicsymmetry){
    ...
    expand(co,nk,kon,ipkon,lakon,ne,nodeboun,ndirboun,xboun,nboun,
	   ...
	   &nherm,xmr,xmi,typeboun,ielprop,prop,orname,itiefac,t0g,t1g,iponoel);
  ...
  **********************************************************************/
  ...
  if(nfour<=0){
	...
	mumps_factor(ad,au,adb,aub,&sigma,icol,irow,&neq[1],&nzs[1],
		     &symmetryflag,&inputformat,jq,&nzs[2]);
#else
	printf(" *ERROR in steadystate: the MUMPS library is not linked\n\n");
	FORTRAN(stop,());
#endif
      }
      else if(*isolver==11){
	...
	accelerate_factor(ad,au,adb,aub,&sigma,icol,irow,&neq[1],&nzs[1],
			  &symmetryflag,&inputformat,jq,&nzs[2]);
#else
	printf(" *ERROR in steadystate: the Apple Accelerate solver is not linked\n\n");
	FORTRAN(stop,());
#endif
      }
    }
...
#ifdef MUMPS
	    mumps_solve(ubr,&neq[1],&symmetryflag,&inputformat,&nrhs);
#endif
	  }
	  else if(*isolver==11){
#ifdef ACCELERATE_SOLVER
	    accelerate_solve(ubr,&neq[1],&symmetryflag,&inputformat,&nrhs);
#endif
	  }
	}else{
...
#ifdef MUMPS
	    mumps_solve(ubi,&neq[1],&symmetryflag,&inputformat,&nrhs);
#endif
	  }
	  else if(*isolver==11){
#ifdef ACCELERATE_SOLVER
	    accelerate_solve(ubi,&neq[1],&symmetryflag,&inputformat,&nrhs);
#endif
	  }
	}else{
	...
	results(co,nk,kon,ipkon,lakon,ne,vr,stnr,inum,
		...
		inoeln,nener,orname,&network,ipobody,xbodyact,ibody,
